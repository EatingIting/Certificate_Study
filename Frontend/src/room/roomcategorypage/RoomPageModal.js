...
const RoomPageModal = ({ open, onClose, study }) => {
  ...
  const hostEmail = String(study.hostUserEmail || study.hostEmail || "")
    .trim()
    .toLowerCase();
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (message.includes("탈퇴한 스터디방은 재가입 할 수 없습니다.")) {
        setApplyError("탈퇴한 스터디방은 재가입 할 수 없습니다.");
      } else if (message.includes("스터디장에 의하여 재가입 불가능 합니다.")) {
        setApplyError("스터디장에 의하여 재가입 불가능 합니다.");
      } else if (message.includes("탈퇴한 스터디에는 다시 가입할 수 없습니다.")) {
        // 구버전 백엔드 메시지 호환
        setApplyError("탈퇴한 스터디방은 재가입 할 수 없습니다.");
      } else {
          ...
          {step === 1 ? (
            <div className="sr2-content">
              ...
              <div className="sr2-info">
                ...
                {isLoggedIn && !genderLoading && !isMyStudy && !isGenderAllowed && (
                  <div
                    style={{
                      marginTop: "3px",
                      fontSize: "13px",
            ...
            <div className="sr2-form">
              ...
              {applyError && <div className="sr2-error">{applyError}</div>}
            </div>
          )}
        </div>
