...
public class ApplicationController {
    ...
    // 신청하기
    @PostMapping
    public ResponseEntity<?> applyToRoom(
    ...
    // 신청 받은 목록
    @GetMapping("/received")
    public List<ApplicationVO> getReceivedApplications(Authentication authentication) {
        if (authentication == null) {
    ...
    // 내가 신청한 목록
    @GetMapping("/sent")
    public List<ApplicationVO> getSentApplications(Authentication authentication) {
        if (authentication == null) {
            throw new RuntimeException("인증 정보가 없습니다.");
        }
    ...
    public void approveApplication(
        ...
        try {
            applicationService.approveApplication(joinId, authentication.getName());
        } catch (IllegalStateException e) {
    ...
    public void rejectApplication(
        ...
        try {
            applicationService.rejectApplication(joinId, authentication.getName());
        } catch (IllegalStateException e) {
            throw new ResponseStatusException(HttpStatus.CONFLICT, e.getMessage());
        }
    }
